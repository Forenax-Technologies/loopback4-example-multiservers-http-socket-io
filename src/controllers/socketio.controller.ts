...
export class SocketIoController {
  ...
  connect(socket: Socket) {
    console.info('Client connected: %s', this.socket.id);
  ...
  async registerChannel(msg: string[]) {
    console.info('Subscribe to channel: %s', msg);
  ...
  handleChatMessage(msg: unknown) {
    console.info('General forwarded message: %s', msg);
  ...
  handleGeneralMessage(msg: string) {
    console.info('General Message : %s', msg);
    const parsedMsg: {
      subject: string;
      body: string;
      receiver: {
  ...
  @socketio.disconnect()
  disconnect() {
    console.info('Client disconnected: %s', this.socket.id);
  }
}
